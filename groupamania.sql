CREATE DATABASE IF NO EXISTS `groupamania`;
...
CREATE TABLE IF NOT EXISTS `posts` (
	...
	CONSTRAINT `FK_posts_users` FOREIGN KEY (`userid`) REFERENCES `groupamania`.`users` (`id`) ON UPDATE NO ACTION ON DELETE NO ACTION
);
...
CREATE TABLE `comments` (
	...
	CONSTRAINT `FK__posts` FOREIGN KEY (`postid`) REFERENCES `groupamania`.`posts` (`id`) ON UPDATE NO ACTION ON DELETE NO ACTION,
	CONSTRAINT `FK__users` FOREIGN KEY (`userid`) REFERENCES `groupamania`.`users` (`id`) ON UPDATE NO ACTION ON DELETE NO ACTION
);